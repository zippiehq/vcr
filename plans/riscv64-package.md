Have to build this with --target=linux/riscv64
...
RUN cargo build --target riscv64gc-unknown-linux-musl --release
...
COPY --from=builder /app/target/riscv64gc-unknown-linux-musl/release/rust-binary /bin/rust-binary
CMD ["/bin/rust-binary"]
```