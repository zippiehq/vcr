...
use vsock_protocol::{
    Packet, VirtioVsockHdr, VSOCK_OP_REQUEST, VSOCK_OP_RESPONSE, VSOCK_OP_RW, VSOCK_OP_SHUTDOWN,
    VSOCK_TYPE_STREAM,
};
        ...
        match receive_packet(machine)? {
            Some(packet) => {
                if packet.hdr().op == VSOCK_OP_RESPONSE {
                    ...
                    info!("Unsuccessful connection attempt, retrying...");
                }
            }
            None => {
                info!("No packet received in response to connection request, retrying...");
            }
        }
